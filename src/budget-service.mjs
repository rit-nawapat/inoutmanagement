...
export function renderBudgetSelector({
  ...
  const allPill = createEl('div', {
    className: `flex items-center justify-center space-x-1.5 max-md:py-2 max-md:px-2 md:py-2 md:px-3.5 max-md:rounded-xl md:rounded-xl text-[10px] max-md:text-[11px] lg:text-xs lg:py-2.5 lg:px-4.5 font-bold border transition-colors cursor-pointer ${noGroupSelected
  ...
  allPill.appendChild(createEl('span', { className: 'truncate min-w-0', text: 'ไม่ใช้กระเป๋า' }));
  ...
  activeGroups.forEach((group) => {
    ...
    pill.appendChild(createEl('span', { className: 'truncate min-w-0', text: `${prefix}${group.name} (฿${group.remaining.toLocaleString()})` }));
