...
export function renderBudgetSelector({
  ...
  const allPill = createEl('div', {
    className: `flex items-center space-x-1.5 max-md:py-2.5 max-md:px-4 md:py-2 md:px-3.5 max-md:rounded-2xl md:rounded-xl text-xs md:text-[10px] lg:text-xs lg:py-2.5 lg:px-4.5 font-bold border transition-colors cursor-pointer ${noGroupSelected
        ? 'bg-slate-900 border-slate-900 text-white shadow-sm'
        : 'bg-white max-md:border-slate-100 md:border-slate-200 text-slate-500 hover:bg-slate-50 max-md:shadow-sm'
      }`,
  });
  allPill.appendChild(createIcon('wallet', 'max-md:w-4.5 max-md:h-4.5 md:w-3.5 md:h-3.5'));
  ...
  activeGroups.forEach((group) => {
    ...
    const pill = createEl('div', {
      className: `flex items-center space-x-1.5 max-md:py-2.5 max-md:px-4 md:py-2 md:px-3.5 max-md:rounded-2xl md:rounded-xl text-xs md:text-[10px] lg:text-xs lg:py-2.5 lg:px-4.5 font-bold border transition-all cursor-pointer ${isSelected
