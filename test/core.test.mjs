...
test('mobile add page uses one scroll container with a fixed bottom save action', () => {
  ...
  assert.match(cssSource, /body\.is-add-page #btn-save\s*\{[\s\S]*box-shadow:\s*0 12px 24px rgba\(15, 23, 42, 0\.16\)/);
  assert.match(cssSource, /#main-touch-zone\s*\{[\s\S]*position:\s*relative/);
  assert.match(cssSource, /body\.is-add-page #main-touch-zone::after\s*\{[\s\S]*content:\s*""/);
  assert.match(cssSource, /body\.is-add-page #main-touch-zone::after\s*\{[\s\S]*background:\s*#ffffff/);
  assert.match(cssSource, /body\.is-add-page #main-touch-zone::after\s*\{[\s\S]*left:\s*0/);
  assert.match(cssSource, /body\.is-add-page #main-touch-zone::after\s*\{[\s\S]*right:\s*0/);
  assert.match(cssSource, /body\.is-add-page #main-touch-zone::after\s*\{[\s\S]*bottom:\s*var\(--mobile-nav-height\)/);
  assert.match(cssSource, /body\.is-add-page #main-touch-zone::after\s*\{[\s\S]*pointer-events:\s*none/);
